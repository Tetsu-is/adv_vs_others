...
from perlcompat import die, warn
import graph_tools
import numpy
...
class SRW:
    ...
    def __init__(self, graph=None, current=None, target=None, rng=None, *kargs, **kwargs):
        self.graph = graph
        ...
        self.discovered_vertices = set()  # 訪問済み + 隣接ノード
        self.current = None
        if current:
    ...
    def pick_next(self, u=None):
        ...
        chosen = self.rng.uniform(0, total)
        accum = 0
        for v in neighbors:
            accum += weights[v]
            if chosen < accum:
    ...
    def move_to(self, v):
        ...
        if not self.nvisits[v]:  # is this the first time?
            ...
            self.ncovered += 1
            # 増分で discovered_vertices を更新（setなので重複は自動排除）
            self.discovered_vertices.add(v)
            self.discovered_vertices.update(self.graph.neighbors(v))
        self.nvisits[v] += 1
    ...
    def get_discovered_graph(self):
        """Return a subgraph of the graph that contains only the vertices
        that the agent has visited."""
        g = graph_tools.Graph(directed=self.graph.is_directed())
        for v in self.graph.vertices():
            if self.nvisits[v]:
                g.add_vertex(v)
        for u, v in self.graph.edges():
            if self.nvisits[u] and self.nvisits[v]:
                g.add_edge(u, v)
        return g

    def get_discovered_graph_with_neighbors(self):
        """Return a subgraph of the graph that contains the vertices
        that the agent has visited and their neighbors, with edges
        only between visited nodes and their neighbors."""
        g = graph_tools.Graph(directed=self.graph.is_directed())

        # discovered_verticesからノードを追加（全頂点イテレート不要）
        for v in self.discovered_vertices:
            g.add_vertex(v)

        # 訪問済みノードからその隣接へのエッジを追加
        for v in self.nvisits:
            if self.nvisits[v] > 0:  # 実際に訪問済みのノードのみ
    ...
    def coverage_with_neighbors(self):
        """Return the coverage ratio including visited nodes and their neighbors.
        This is O(1) as it uses pre-computed discovered_vertices set."""
        return len(self.discovered_vertices) / self.graph.nvertices()
...
class LZRW(BiasedRW):
    ...
    def pick_next(self, u=None):
        ...
        if u is None:
            u = self.current
        if self.rng.random() <= self.laziness:
            return u
        else:
